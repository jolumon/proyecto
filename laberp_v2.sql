...
CREATE TABLE lotes_ubicados(
    mp_lubi int not null,
    id_ubi_lubi int not null,
    lote_lubi varchar(10) not null,
    cantidad_lubi decimal(10,2) not null,
    CONSTRAINT cantidad_posit CHECK (cantidad_lubi>=0),
    CONSTRAINT pk_lubi PRIMARY KEY (mp_lubi, id_ubi_lubi,lote_lubi),
	CONSTRAINT fk1_lubi FOREIGN KEY (mp_lubi) REFERENCES materias_primas(id_mp),
	CONSTRAINT fk2_lubi FOREIGN KEY (id_ubi_lubi) REFERENCES ubicaciones(id_ubi)
);

INSERT INTO lotes_ubicados (mp_lubi, id_ubi_lubi, lote_lubi,cantidad_lubi) values (1,2,'L001',100),
    (2,2,'L001',100),
    (3,2,'L001',100);
...
CREATE TABLE ordenes_fab (
    id_fab bigserial NOT NULL,
	id_prod_fab int NOT NULL,
	fecha_fab date NOT NULL,
	lote_fab varchar(10) NOT NULL,/*No haría falta, se podría identificar con la pk*/
	fecha_cad_fab date NULL,
	equipo_fab int4 NULL,
	cantidad_fab int4 NULL,
	indicaciones_fab varchar(1000) NULL,
	CONSTRAINT ch CHECK ((fecha_fab < fecha_cad_fab)),
	CONSTRAINT pk_fab PRIMARY KEY (id_fab),
	CONSTRAINT fk_fab1 FOREIGN KEY (id_prod_fab) REFERENCES productos(id_prod),
	CONSTRAINT fk_fab2 FOREIGN KEY (equipo_fab) REFERENCES equipos(id_eq)
);

INSERT INTO ordenes_fab(id_prod_fab,fecha_fab,lote_fab,fecha_cad_fab,equipo_fab,cantidad_fab,indicaciones_fab)
VALUES
	 ...
	 (2,'2023-11-11','1465sdfd',NULL,4,300,NULL),
	 (2,'2023-11-17','abc','2024-01-01',4,300,NULL);
	 
CREATE TABLE componentes_fab (
	id_fab_comp serial4 NOT NULL,
	id_mp_comp int4 NOT NULL,
	id_prov_comp int4 NOT NULL,
	cantidad float not null,
	CONSTRAINT pk_comp_fab PRIMARY KEY (id_fab_comp),
	CONSTRAINT fk1_comp_fab FOREIGN KEY (id_mp_comp,id_prov_comp) REFERENCES composiciones(id_prod_comp, id_mp_comp)
